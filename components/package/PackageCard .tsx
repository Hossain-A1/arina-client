...
const PackageCard: React.FC<PackageCardProps> = ({ packageitem }) => {
  return (
    <div className='w-full rounded-xl bg-white p-5 shadow-sm shadow-gray'>
      <Link href={`/packages/${packageitem._id}`} className='inline-block w-full h-[20rem] overflow-hidden rounded-xl group'>
        <Image
...
<p className="text-black/50">{packageitem.description.substring(0,50)}...</p>
<div className="flex gap-2.5 justify-between items-center">
<h3>${packageitem.price}</h3>
<Link href={`/packages/${packageitem._id}`} className={cn(buttonVariants({variant:"danger"}))}>View details</Link>
</div>
      </div>
    </div>
  );
